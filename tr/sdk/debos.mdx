...
isimli dosyayı okuyarak 4 aşamalı bir çalışma gerçekleştirir.

<ParamField body="1. Aşama" type="Debootstrap/Apt">
Parametre olarak verilen Dağıtım türüne göre (Ubuntu veya Debian) debootstrap aracıyla ilgili dağıtımların
paket (apt) sunucularından **.deb** uzantılı bütün paketleri indirir.
</ParamField>

<ParamField body="2. Aşama" type="Paketlerin Kurulumu">
İndirilen **.deb** uzantılı paketlerin kurulumu için `rootfs` ismi verilen bir klasör yapısı oluşturarak GNU/Linux
işletim sistemi gereksinimlerini ayarlayarak paketlerin kurulumunu `rootfs` içerisine `chroot` yöntemi ile yapar.
</ParamField>

<ParamField body="3. Aşama" type="Özelleştirme İşlemleri">
...
<ParamField body="4. Aşama" type="İmaj Dosyası Oluşturma">
Yukarıdaki bütün işlemler başarılı bir biçimde tamamlandıktan sonra `image-partition` türündeki tanımlamaya göre
boot, rootfs disk bölümlendirmeleri yapılır, **.img** uzantılı dosya oluşturulur ve bütün dosyalar içerisine taşınır.
</ParamField>
