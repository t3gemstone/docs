...
## Güç bağlantısı
  ...
  <Columns cols={2}>
  <Frame caption="USB-C ile">
    <img className="rounded-lg" src="/images/o1-board/31-usb-connection.jpg" />
  </Frame>
  <Frame caption="Harici güç kaynağı ile">
    <img className="rounded-lg" src="/images/o1-board/28-power-with-battery.jpg" />
  </Frame>
</Columns>

Gemstone O1 geliştirme kartının harici güç girişi 5V–12V / 5A aralığında çalışacak şekilde tasarlanmıştır. Bu aralıkta herhangi bir uygun harici güç kaynağı bağlanabilir. USB tarafında ise USB PD (Güç Dağıtım) entegresi 9V’a ayarlanmıştır. Eğer bağlı cihaz 9V desteğine sahipse, geliştirme kartı 9V / 3A ile beslenir; cihaz uygun değilse kart, 5V / 3A ile güç almaya devam eder.
...
Kartın açılıp açılmadığını, düzgün çalışıp çalışmadığını ve USB
portlarının sağlıklı olup olmadığını ekrana veya log'a bakmadan
anlayabilmek için kartın üzerinde 6 adet LED bulunmaktadır.

<Frame caption="Kartın üzerindeki LED'ler">
  <img className="rounded-lg" src="/images/o1-board/39-board-leds-tr.jpg" />
</Frame>

#### 1.1. Sistem LED'i (sağ) — PWR / ACT

Kart açılırken `PWR` LED'i kırmızı yanmaktadır. Sistem tamamen açıldığında ise `ACT` LED'i
yeşile dönerek düzenli aralıklarla yanıp sönmektedir - bu, sistemin sorunsuz çalıştığına
işaret etmektedir. Yanıp sönmenin durması halinde sistem kilitlenmiş demektir.

#### 1.2. Kullanıcı LED'i (sol) — GREEN / RED

Açılış sırasında `RED` LED'i kırmızı yanarken sistem tamamen açıldığında sönmektedir.
Açılıştan sonra sistem kullanıcı LED'lerini kullanmamaktadır; isterseniz
`GREEN` ve `RED` LED'lerini kendinize göre konfigüre edebilirsiniz.

```bash
echo 1 | sudo tee /sys/class/leds/GREEN/brightness   # yak
echo 0 | sudo tee /sys/class/leds/GREEN/brightness   # söndür
```

```bash
echo 1 | sudo tee /sys/class/leds/RED/brightness   # yak
echo 0 | sudo tee /sys/class/leds/RED/brightness   # söndür
```

#### 1.3. USB Güç LED'leri

USB portlarının yanındaki iki yeşil LED, kartın güç girişinde
bir sorun (aşırı akım/gerilim) olup olmadığını göstermektedir.
Güç hattında sorun oluştuğunda ise LED'ler sönmektedir.

#### 1.4. Boot Hatasında LED Durumu

`PWR` (sağ) ve `RED` (sol) LED'leri uzun bir süre
sönmeden kırmızı yanmaya devam etmekteyse, sistem boot sırasında
takılmış demektir. Önce kartın arkasındaki
[Bootmode Switch](/tr/quickstart#1-3-bootmode-switch) kombinasyonunu
kontrol edebilirsiniz. Sorun switch'lerden kaynaklanmıyorsa, karta TTL
üzerinden konsol bağlantısı kurup boot sırasındaki hata mesajlarını inceleyiniz.

## 2. Pinout

<Frame caption="Kartın GPIO pinlerini belirten görsel">
...
konfigürasyonda hangi arayüzlere tahsis edildiğine değinilecek, sonraki bölümde ise [Device Tree Overlay](/tr/faq#device-tree-overlay-nedir)
dosyaları ile pinlerin nasıl farklı kullanımlara açılabileceği izah edilecektir.

Örneğin `GPIO-7` isimli pin varsayılan konfigürasyonda `SPI-MCU0 CS2` olarak belirlenmiş
olup [Device Tree Overlay](/tr/faq#device-tree-overlay-nedir) ismi verilen dosya aktif edilerek `UART-WKUP0 RX` fonksiyonuna dönüştürülebilmektedir.
...
<SnippetPinout func="İŞLEV" pins="PİNLER" />
...
Bu SPI arayüzüne Gemstone kartta yer alan `/dev/spidev0.0` ve `/dev/spidev0.2` cihazları üzerinden erişilebilmektedir.
`/dev/spidev0.0` cihazı SPI-MCU0 CS0, `/dev/spidev0.2` cihazı ise SPI-MCU0 CS2'e karşılık gelmektedir.
...
[Device Tree Overlay](/tr/faq#device-tree-overlay-nedir) adı verilen dosyalarda tanımlanmıştır. İstenen konfigürasyonun/konfigürasyonların aktif
...
### 3.1. Örnek: UART6 sinyallerinin 40-pin HAT'e yönlendirilmesi
...
#### 3.1.2. İlgili Overlay Dosyasının Bulunması

Sonraki aşamada istenen konfigürasyonu hangi [Device Tree Overlay](/tr/faq#device-tree-overlay-nedir) dosyasının yaptığının
bulunması gerekmektedir, bu dosyalarının tümü geliştirme kartının içerisindeki `/boot/overlays` klasöründe bulunmaktadır.
`ls /boot/overlays` komutu çalıştırılarak hepsi listelenebilir.

<SnippetDeviceTreeOverlays />

#### 3.1.3. uEnv.txt Dosyasının Güncellenmesi
