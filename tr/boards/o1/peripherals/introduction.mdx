...
portlarının sağlıklı olup olmadığını ekran veya log'a bakmadan
anlayabilmek için kartın üzerinde 6 LED bulunur.
...
Kart açılırken kırmızı (`PWR`) yanar. Linux tamamen açıldığında yeşile
(`ACT`) döner ve düzenli aralıklarla yanıp söner — bu, sistemin
sorunsuz çalıştığının işaretidir. Yanıp sönme durursa sistem
kilitlenmiş demektir.

#### 1.2. Kullanıcı LED'i (sol) — GREEN / RED

Kart açılırken kısa süreliğine kırmızı (`RED`) yanar, ardından söner.
Sistem bu LED'i kullanmaz; isterseniz `GREEN` ve `RED` LED'lerini kendi
yazılımınızdan yakıp söndürebilirsiniz.
...
USB portlarının yanındaki iki yeşil LED, o portun güç hattında bir
sorun (aşırı akım/gerilim) olup olmadığını gösterir. Normalde ikisi de
yanık kalır; gerçek bir sorun oluşursa söner.
