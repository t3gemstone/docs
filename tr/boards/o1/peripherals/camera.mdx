...
- Gemstone Obsidyen geliştirme kartına nasıl CSI kamera bağlayabileceğinizi.
- Overlay eklemeyi.
</Tip>
...
Kamera ile Gemstone Obsidyen geliştirme kartı arasındaki kablonun doğru bir şekilde bağlanması için aşağıda yer alan fotoğraftaki gibi bir bağlantı yapmanız gerekmektedir.
...
Bir önceki komutla elde ettiğiniz dosya adını `overlays=` satırının sonuna boşluk bırakarak ekleyin. Örneğin CSI0 (ethernete uzak olan konnektör) konnektörüne IMX219 kamerasını bağladığınızda
```
...
`gemstone-camera-setup` scripti ile karta bağlı olan kameraların kurulumu sağlanmalıdır.
...
Başlangıçta aşağıdaki komut, bir kamera sensörünün çekirdek sürücüsü (kernel driver) ile kullanıcı alanı (user space) uygulamaları arasında doğru bir iletişim kurulmasını sağlamak için kullanılır.
