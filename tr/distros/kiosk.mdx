...
```bash
pc@host:~$ scp kiosk gemstone@10.0.0.1:bin/kiosk
pc@host:~$ ssh gemstone@10.0.0.1 DISPLAY=:0.0 openbox --exit
```

Ekranınız için geçerli çözünürlük ve yenileme hızı değerlerini aşağıdaki komut ile listeleyebilirsiniz.

```bash
pc@host:~$ ssh gemstone@10.0.0.1 DISPLAY=:0.0 xrandr --query
