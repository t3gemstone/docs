...
<Tabs>
  <Tab title="Windows">
    <Frame caption="Todo: Set IP address from Control Panel">
      <img height="200" width="200" src="/images/under-construction.png" />
    </Frame>
  </Tab>
  <Tab title="Linux">
    <Frame caption="Todo: Set IP address from CLI or gnome-settings GUI">
      <img height="200" width="200" src="/images/under-construction.png" />
    </Frame>
  </Tab>
  <Tab title="macOS">
...
After setting your computer's IP, get a shell from Gemstone board via serial port and run the commands below.
A new connection for static IP will be created and its priority will be set higher than DHCP via `nmcli` tool. That way 
static IP will be assigned automatically.
...
```bash
host@pc:~$ git clone https://github.com/t3gemstone/examples.git
host@pc:~$ cd examples
```
...
```bash
host@pc:examples$ ./setup.sh
host@pc:examples$ devbox shell
host@pc:examples$ task fetch
```

#### 2.2.4. Cross compile an example project using the toolchain

```bash
host@pc:examples$ PROJECT=magnetometer task build
```

#### 2.2.5. Upload compiled project into the development board

```bash
host@pc:examples$ scp -r build/examples gemstone@10.0.0.1:build
```
