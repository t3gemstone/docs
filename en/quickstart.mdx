...
After completing the setup, run the program and connect to your card by selecting the connection type shown in the screenshot below.

<Frame caption="Putty startup screen" >
  <img src="/images/putty-terminal.png" />
</Frame>

Right-click on the Computer icon, select Manage → Device Manager, identify which serial port has been created, and enter it into PuTTY. The baud rate for the connection is 115200.

For example: COM9, 115200

When you connect to the serial port, the Putty terminal screen may appear as follows. This means that the terminal connection was not established before the card booted up. To restart the connection, press the reset button on the card to reset it and establish the terminal connection.

<Frame caption="Putty Terminal Connection">
  <img src="/images/Putty-first-opening.png" />
</Frame>

To reset your card, simply press the button shown in the image below.

<Frame caption="Gemsotone Reset Button">
  <img src="/images/Gemstone-reset-button.png" />
</Frame>

To connect directly to the terminal, it is recommended that you first open the putty terminal, then power the card.
