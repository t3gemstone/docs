...
Agents can communicate through an access point or via **IBSS ad-hoc mode**, which requires no
central infrastructure.

#### Ad-hoc Network Setup (IBSS)

IBSS (Independent Basic Service Set) mode lets agents communicate directly without a central access
point. This is the recommended approach for field use where no AP is available. Setup is done with
the `iw` tool; refer to the official Linux Wireless documentation for the steps:

- [IBSS / Ad-hoc Network Setup with iw — Linux Wireless](https://wireless.wiki.kernel.org/en/users/documentation/iw)

#### Communication Protocol

Position and state messages are exchanged over UDP. Each agent periodically broadcasts its own
state; the others listen and use the received data for formation calculations:

| Field | Type | Description |
|-------|------|-------------|
| `id` | int | Agent number (1–N) |
| `lat`, `lon` | float | GPS position (degrees) |
| `alt` | float | Altitude (m, relative) |
| `vx`, `vy` | float | Horizontal velocity (m/s) |
...
### 2.7. Swarm Algorithm and ArduPilot GUIDED Mode

#### Putting ArduPilot into GUIDED Mode

For the swarm software to send position commands to ArduCopter, the vehicle must be in **GUIDED**
flight mode. This mode accepts `SET_POSITION_TARGET_*` MAVLink messages from external software.

Each vehicle must also be assigned a unique MAVLink system ID (`SYSID_THISMAV`); without this,
all agents on the same network will respond to each other's commands. For how to enable GUIDED
mode, set parameters, and send position commands, refer to the official ArduPilot documentation:

- [GUIDED Mode — ArduCopter Docs](https://ardupilot.org/copter/docs/ac2_guidedmode.html)
- [MAVLink Commands in GUIDED Mode](https://ardupilot.org/dev/docs/copter-commands-in-guided-mode.html)

#### Leader-Follower Formation Algorithm

The most common approach is the **leader-follower** model. The leader agent follows the mission
route; followers position themselves at a fixed offset relative to the leader. Each agent
broadcasts its GPS position over UDP; others receive it and compute their target positions.

For formation algorithms, multi-vehicle setup, and example code, refer to the ArduPilot developer
guide:

- [Flying with Multiple Vehicles — ArduCopter Docs](https://ardupilot.org/copter/docs/common-multi-vehicle-flying.html)
...
flowchart TB
    ...
    SW1 <-->|"WiFi (position, formation)"| SW2
    SW1 <-->|"WiFi"| SW3
    ...
    GCS["Ground Station (Test / Monitoring)"] <-->|"MAVLink / UDP"| AGENT1
```
