...
Real vehicle testing requires access to a field, equipment, and safety conditions that are not always available at every stage of development. ROS 2 stacks running on T3 Gemstone O1 can be tested directly with Gazebo or CARLA simulators, allowing algorithms to be validated before being deployed to the real vehicle.
