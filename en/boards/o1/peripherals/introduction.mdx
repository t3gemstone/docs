...
— without needing to check a screen or the logs.
...
The `PWR` LED lights up red as the board powers on. Once the system
has fully booted, the `ACT` LED turns green and blinks at a steady
interval, indicating that the system is running normally. If the
blinking stops, the system has locked up.

#### 1.2. User LED (left) — GREEN / RED

The `RED` LED lights up red during boot and turns off once the system
has fully booted. After boot, the system doesn't use the user LEDs;
you can configure `GREEN` and `RED` however you like.
...
The two green LEDs next to the USB ports indicate whether there's a
fault (over-current/over-voltage) on the board's power input. The
LEDs turn off when a fault occurs on the power line.

#### 1.4. LED State on a Boot Failure

If the `PWR` (right) and `RED` (left) LEDs stay lit red for an
extended period without turning off, the system has gotten stuck
during boot. You can first check the
[Bootmode Switch](/en/quickstart#1-3-bootmode-switch) combination on
the back of the board. If that's not the issue, please connect to the
board over a TTL console connection and check the boot error messages.
