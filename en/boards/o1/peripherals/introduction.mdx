...
The Gemstone Obsidian development board's external power input is designed to operate within the 5V–12V / 3A range. Any suitable external power source within this range can be connected. On the USB side, the USB PD (Power Delivery) integrator is set to 9V. If the connected device supports 9V, the development board is powered at 9V / 3A; if the device is not compatible, the board continues to draw power at 5V / 3A.
...
<Danger>
When no USB connection, the development board is protected against reverse polarity connection to the 12V battery. However, this protection does not provide complete assurance, so the user must connect it carefully.
</Danger>
