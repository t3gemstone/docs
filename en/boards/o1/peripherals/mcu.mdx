...
### 3.2 Example Scenario: Driving a Servo Motor from the MCU using PWM

Consider a scenario where an SG90 model servo motor must be driven from an MCU core. First, select a PWM pin available on the HAT. Suppose we choose
the `GPIO-12` pin. Consulting the table reveals that this pin is used by Linux. To disable it, check the How to Disable column, which lists a single
overlay file. To disable it, open the `/boot/uEnv.txt` file and inspect the `overlays` variable.
...
Locate the overlay `k3-am67a-t3-gem-o1-pwm-ecap0-gpio12.dtbo` as indicated in the table and remove it. The overlays variable should now appear as
follows:
```txt
